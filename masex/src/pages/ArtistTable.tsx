...
export default function ArtistTable() {
    const [allArt, setAllArt] = useState<Artist[]>([])
    ...
    const DelArtist = (id: string) => {
        DeleteArtist(id)
        setUpdate(true)
    }

    function CreateForm() {
        const [values, setValues] = useState<Artist>(defaultArtistForm);
    ...
    function UpdateForm({ artist, artistId }: { artist: Artist; artistId: string }) {
    ...
    return (
        <Stack style={{ alignItems: 'center', padding: '0rem 8rem' }}>
            <h1>Mason Exhibitions Artist</h1>
            <Stack width={"100%"}>
                <CreateForm />
            </Stack>
                    ...
                    {allArt.map((row, index) => (
                        <React.Fragment key={index}>
                            ...
                            >
                                ...
                                <TableCell align="right"><IconButton onClick={() => openScroll(index)}><EditIcon /></IconButton><IconButton onClick={() => DelArtist(String(row.base?.id) || "")}><DeleteIcon /></IconButton></TableCell>
                            </TableRow>
                            {open === index && (
                                <TableRow ref={editRowRef}>
                                    <TableCell colSpan={12}>
                                        <Stack direction={'row-reverse'} alignItems={'flex-start'}>
                                            ...
                                            <UpdateForm artist={row} artistId={String(row.base?.id) || ""} />
